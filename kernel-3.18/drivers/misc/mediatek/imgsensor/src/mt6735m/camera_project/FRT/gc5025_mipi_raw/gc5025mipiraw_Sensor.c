...
#include "gc5025mipiraw_Sensor.h"
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
    ...
    kal_uint8 retry = 2;
    	...
    	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
			...
			printk("gc5025mipiraw_Sensor i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
                	return ERROR_NONE;
            	}
		printk("gc5025mipiraw_Sensor Read sensor id fail, write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
            	retry--;
        	} while(retry > 0);
        	i++;
        	retry = 2;
